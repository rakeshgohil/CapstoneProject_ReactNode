...
class FileUpload:
    ...
    def save_file_and_generate_secret(self, file, userids, userid):
        ...
        try:
            ...
            self.cursor.execute('''
                EXEC sp_SaveFile @FileName=?, @FilePath=?, @UserId=?, @Secret=?, @SharedUserIDs=?
            ''', (filename, file_path, userid, secret, cleaned_userids))
            self.conn.commit()
        ...
        try:
            ...
            for i in range(5):
                ...
                self.cursor.execute('''
                    INSERT INTO FileSecrets (FileID, UserId, SecretPart) 
                    VALUES (?, ?, ?);
                ''', (fileid, cleaned_userids[i].strip(), shares[i]))
                self.conn.commit()            
